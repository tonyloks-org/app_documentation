...
7. **Avito** — доступ к статистике, балансу и контенту объявлений.
...
Для подключения к Avito API:

1. **Введите название подключения**  
   Например, «Avito основной аккаунт».

2. **Добавьте учетные данные**  
   - `Client ID` — идентификатор интеграции Avito.  
   - `Client Secret` — секрет интеграции.  
   - `User ID` — ID профиля Avito (главный аккаунт).  
   - Скриншот 1: _где посмотреть ID профиля_ (вставить после подготовки).  
   - Скриншот 2: _где взять Client ID / Client Secret_ (вставить после подготовки).

3. **Сохраните настройки**  
   - После валидации подключение появится в списке.
